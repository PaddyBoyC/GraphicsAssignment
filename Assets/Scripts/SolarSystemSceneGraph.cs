...
public class SolarSystemSceneGraph
{
    ...
    public SolarSystemSceneGraph(MyVector pPosition, MyVector pRotation, MyVector pScale, Material sunMaterial, Material mercuryMaterial, Material venusMaterial, Material earthMaterial, Material moonMaterial, Material marsMaterial, Material jupiterMaterial, Material saturnMaterial, Material uranusMaterial, Material neptuneMaterial)
    {
        Position = pPosition;
        Rotation = pRotation;
        Scale = pScale;
        ...
        Planet sun = new Planet(new MyVector(0, 0, 0), oneVector.Multiply(5), new MyVector(0, 0, 0), 10000, Color.yellow, sunMaterial);
        rootScaleNode.AddChild(sun.RootNode);
        bodies.Add(sun);

        Planet mercury = new Planet(new MyVector(5, 0, 0), oneVector.Multiply(0.20f), new MyVector(0, 0, 0), 1, Color.blue, mercuryMaterial);
        rootScaleNode.AddChild(mercury.RootNode);
        bodies.Add(mercury);

        Planet venus = new Planet(new MyVector(6, 0, 0), oneVector.Multiply(0.50f), new MyVector(0, 0, 3), 1, Color.blue, venusMaterial);
        ...
        Planet moon = new Planet(new MyVector(7.6f, 0, 0), oneVector.Multiply(0.15f), new MyVector(0, 0, 3), 1f, Color.gray, moonMaterial);
        rootScaleNode.AddChild(moon.RootNode);
        bodies.Add(moon);

        Planet mars = new Planet(new MyVector(10, 0, 0), oneVector.Multiply(0.50f), new MyVector(0, 0, 3), 1, Color.blue, marsMaterial);
        rootScaleNode.AddChild(mars.RootNode);
        bodies.Add(mars);

        Planet jupiter = new Planet(new MyVector(13, 0, 0), oneVector.Multiply(2f), new MyVector(-1, 0, 0), 1, Color.blue, jupiterMaterial);
        rootScaleNode.AddChild(jupiter.RootNode);
        bodies.Add(jupiter);

        Planet saturn = new Planet(new MyVector(18, 0, 0), oneVector.Multiply(1.5f), new MyVector(0, 0, 3), 1, Color.blue, saturnMaterial);
        rootScaleNode.AddChild(saturn.RootNode);
        bodies.Add(saturn);

        Planet uranus = new Planet(new MyVector(26, 0, 0), oneVector.Multiply(1f), new MyVector(0, 0, 3), 1, Color.blue, uranusMaterial);
        rootScaleNode.AddChild(uranus.RootNode);
        bodies.Add(uranus);

        Planet neptune = new Planet(new MyVector(28, 0, 0), oneVector.Multiply(1f), new MyVector(0, 0, 3), 1, Color.blue, neptuneMaterial);
    ...
    public void Update()
    {
        ...
        foreach (var body in bodies)
        {
            ...
            if (planet != null)
            {
                if (LineSphereIntersection(cameraPos, mouseWorldPos2, body.Position, planet.Scale.X))
                {
                    Debug.DrawLine(Camera.main.transform.position, new Vector3(body.Position.X, body.Position.Y, body.Position.Z), Color.red);
                }
                if (PlaneSphereIntersection(new MyVector(0, 0, 0), new MyVector(0, 1, 0), body.Position, planet.Scale.X))
                {
                    Debug.DrawLine(Camera.main.transform.position, new Vector3(body.Position.X, body.Position.Y, body.Position.Z), Color.green);
                }
            }
        }
        RootNode.Draw(MyMatrix.CreateIdentity());
    ...
    bool PlaneSphereIntersection(MyVector planePoint, MyVector planeNormal, MyVector sphereOrigin, float sphereRadius)
    {
        float d = (planeNormal.DotProduct(sphereOrigin.Subtract(planePoint)) / planeNormal.Magnitude());
        return Mathf.Abs (d) <= sphereRadius;
    }
}
